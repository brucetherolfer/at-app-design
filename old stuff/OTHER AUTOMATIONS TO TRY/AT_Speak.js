// AT Speak - Scriptable (iOS)
// Picks a random AT prompt and speaks it aloud via iOS text-to-speech
// Respects waking hours (7am-9pm) and massage blackout windows
//
// SETUP:
// 1. Save this in Scriptable as "AT Speak"
// 2. Open Shortcuts app → Automation tab
// 3. New Automation → Time of Day → set to 7:00am, repeat Every Hour (or Every 15 Min)
// 4. Add action: "Run Scriptable Script" → select "AT Speak"
// 5. Turn OFF "Ask Before Running" and OFF "Notify When Run"
// 6. The script handles its own timing/blackout logic internally
//
// TIP: Create multiple automations at different intervals for more frequent prompts
// e.g. one every 15 min starting 7am, one every 15 min starting 7:07am, etc.
// This gives you ~7-8 min effective spacing with random feel
...
const prompts = [
  ...
  "Is your neck tight?","Let your weight be supported.","Are you lifting your chest unnecessarily?",
  "Notice if you are anticipating the next thing.","Is your head free to nod?",
  ...
  "Are you bracing for something that is not happening?","Notice the space between your shoulder blades.",
...
function isBlackedOut() {
  ...
  return (blackout[dow]||[]).some(([s,e]) => mins >= s && mins < e);
}
